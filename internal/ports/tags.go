...
func MakeGetTagsHandler(
	...
	handler := func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			...
			logger.Info("Invalid uuid. Returning error", "statusCode", statusCode, "reason", "invalid uuid")
		...
		if errors.Is(err, domain.ErrTemporarilyUnavailable) {
			logger.Error("Tags temporarily unavailable", "error", err)
			http.Error(w, "Temporarily unavailable", http.StatusServiceUnavailable)
			return
		} else if err != nil {
			logger.Error("Error getting tags", "error", err)
		...
		if err != nil {
			logger.Error("Failed to convert tags to response", "error", err)
